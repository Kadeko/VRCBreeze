...
namespace VRCBreeze
{
    ...
    [AddComponentMenu("VRC Hair Breeze Creator")]
    public class VRCHairBreezeCreator : MonoBehaviour
    {
        #region Variables
        [SerializeField, Header("Avatar Components:"), Tooltip("Assign your Head bone here.")]
        private GameObject avatarHead;

        [Space, SerializeField, Header("Wind Settings:"), Tooltip("How strong is the wind."), Min(0f)]
        private float windStrength = 10f;

        [SerializeField, Tooltip("If enabled, Bone keyframes are slightly randomized around the middle of the AnimationClip. If disabled, Bone keyframes are placed in the middle of the AnimationClip.")]
        ...
        [Space, SerializeField, Header("Debug:")]
        ...
        private void CreateBreezeAnimation(Direction direction)
        {
            ...
            string pathAsset = $"Assets/VRCBreeze/Animations/Generated/{this.transform.parent.name}_{clipName}.anim";
        ...
        private void AssignAnimationsToController()
        {
            ...
            BlendTree hairBreezeTree = null;
            foreach (var child in blendTree.children)
            {
                if (child.motion is BlendTree bt && bt.name.ToLower() == "hair breeze tree")
                {
                    hairBreezeTree = bt;
                    break;
                }
            }
            if (hairBreezeTree == null)
            {
                Debug.LogError($"{PREFIX} Missing 'Hair Breeze Tree' BlendTree! Make sure you have not changed anything, including names, inside 'FX_Layer'! Use 'FX_Layer (BACKUP)' to solve the problem.");
                return;
            }

            // Hair Breeze World

            BlendTree hairBreezeWorld = null;
            foreach (var child in hairBreezeTree.children)
            {
                if (child.motion is BlendTree bt && bt.name.ToLower() == "hair breeze world")
                {
                    hairBreezeWorld = bt;
                    break;
                }
            }
            if (hairBreezeWorld == null)
            {
                Debug.LogError($"{PREFIX} Missing 'Hair Breeze World' BlendTree! Make sure you have not changed anything, including names, inside 'FX_Layer'! Use 'FX_Layer (BACKUP)' to solve the problem.");
                return;
            }

            var worldChildren = hairBreezeWorld.children;
            if (worldChildren.Length < 4)
            {
                Debug.LogError($"{PREFIX} 'Hair Breeze World' BlendTree has not enough empty motions! Make sure you have not changed anything, including names, inside 'FX_Layer'! Use 'FX_Layer (BACKUP)' to solve the problem.");
                return;
            }
            ...
            hairBreezeWorld.children = worldChildren;

            // Hair Breeze Local

            BlendTree hairBreezeLocal = null;
            foreach (var child in hairBreezeTree.children)
            {
                if (child.motion is BlendTree bt && bt.name.ToLower() == "hair breeze local")
                {
                    hairBreezeLocal = bt;
                    break;
                }
            }
            if (hairBreezeLocal == null)
            {
                Debug.LogError($"{PREFIX} Missing 'Hair Breeze Local' BlendTree! Make sure you have not changed anything, including names, inside 'FX_Layer'! Use 'FX_Layer (BACKUP)' to solve the problem.");
                return;
            }

            var localChildren = hairBreezeLocal.children;
            if (localChildren.Length < 5)
            {
                Debug.LogError($"{PREFIX} 'Hair Breeze Local' BlendTree has not enough empty motions! Make sure you have not changed anything, including names, inside 'FX_Layer'! Use 'FX_Layer (BACKUP)' to solve the problem.");
                return;
            }
            ...
            hairBreezeLocal.children = localChildren;
        ...
        private void SetConstraints()
        {
            ...
            if (avatarHead == null)
            {
                Debug.LogError($"{PREFIX} Missing Avatar's head! Assign your Head bone.");
                return;
            }
            ...
            {
                SourceTransform = avatarHead.transform,
                Weight = 1f
            });
    ...
    #region Editor
    [CustomEditor(typeof(VRCHairBreezeCreator))]
    public class VRCBreezeEditor : Editor
    {
        ...
        {
            VRCHairBreezeCreator creator = (VRCHairBreezeCreator)target;
